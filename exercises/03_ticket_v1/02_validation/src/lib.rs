...
impl Ticket {
    ...
    fn new(title: String, description: String, status: String) -> Self {
        //Enum says "hi!" So does array: will check canonical solution for if/elses.
        let valid_status = [
            String::from("To-Do"),
        ...
        //Maybe we should be returning results by this point...
        if title.is_empty() {
            panic!("Title cannot be empty")
        }

        //We're explicitly counting bytes, but we could (should?) instead count characters,
        //if we'd talked about that type before.
        //Good ol' char*...
        if title.len() > 50 {
